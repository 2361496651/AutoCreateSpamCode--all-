...
//垃圾代码输出路径
NSString *outDirectory = @"/Users/zengchunjun/Desktop/SpamCode2";
//NSString *outDirectory = @"/Users/zengchunjun/Desktop/123456/123456/Test";


//引入的头文件
NSString *PublicontrollHeader = @"APubliControllerHeader.h";


NSString *apphppContent = @"";
NSString *appcppContent = @"";
...
//在文件末尾换号写入数据
void writeDataToStartOfFile(NSString *filePath,NSString *dataString);
//在文件末尾换号写入数据
void writeDataToEndOfFile(NSString *filePath,NSString *dataString);

NSString * getRandomStringsWithLow(int length);

void generateSpamTVCFile(NSString *controllerDir,NSString *className,BOOL isTable);//txt

void generateSpamVCFile(NSString *viewDir,NSString *className);//cpp
...
//        NSString *appreviewInclue = [NSString stringWithFormat:@"#include \"%@.hpp\"\n",className];
//        writeDataToStartOfFile(apphppFilePath, appreviewInclue);
        
        
        
        //创建输出目录
        NSString *ocDir = [outDirectory stringByAppendingPathComponent:@"objective-c"];
        createDirectory(ocDir);
    
        NSString *controllerDir = [outDirectory stringByAppendingPathComponent:@"ControllerDir"];
        createDirectory(controllerDir);
        
        NSString *viewDir = [outDirectory stringByAppendingPathComponent:@"UIViewDir"];
        ...
        int count = 150;
        while (count > 0) {
            NSString *className = getRandomClassName(@"FA_");//随机字符串
            //nsobject
            generateSpamCodeFile(ocDir,className);
            
            if (arc4random() % 5 == 0) { //减少控制器的数量
                //tableviewController
                NSString *tabclassName = getRandomClassName(@"FA_");//随机字符串
                generateSpamTVCFile(controllerDir,[tabclassName stringByAppendingString:@"tableviewController"],YES);
                
                //controller
                NSString *vcClassName = getRandomClassName(@"FA_");//随机字符串
                generateSpamTVCFile(controllerDir,[vcClassName stringByAppendingString:@"controller"],NO);
            }
        ...
        generateSpamCodeFile(ocDir, publicCallClassName);//生成一个唯一的类，用来外界调用
...
{
    int length = (arc4random() % 5) + 2;
    
    //获取随机字符串
    NSString *randomStr = getRandomStringsWithLow(length);
    
    NSString *string = [NSString stringWithFormat:@"%@_%@_",randomStr,getRandromString()];
...
#pragma mark:随机获取字符串  类名方法名之类的尽量不要用这个
NSString * getRandromString(){
    
    NSString *string = @"temp";
    
    NSArray<NSString *> *strings = @[@"parameter",@"team",@"apple",@"wangyi",@"view",@"message",@"tencent",@"abcd",@"game",@"flipped",@"crush",@"nostalgia",@"personal",@"ephemeral",@"effervescence",@"evanescence",@"rendezvous",@"reminisce",@"bazinga",@"others",@"serendipity",@"mellifluous",@"petrichor",@"center",@"eudemonia",@"mother",@"passion",@"eternity",@"fantastic",@"freedom",@"tranquility",@"sweetheart",@"gorgeous",@"contacts",@"sophisticated",@"renaissance",@"games",@"cosmopolitan",@"bumblebee",@"umbrella",@"nexts",@"flabbergasted",@"hippopotamus",@"moduel",@"smashing",@"loquacious",@"smithereens",@"hodgepodge",@"shipshape",@"network",@"sources",@"explosion",@"fuselage",@"zing",@"believe",@"Smithereens",@"finals",@"Galaxy",@"Butterfly",@"Rainbow",@"Destiny"];
...
// Created by apple on %@\n\
// \n\
// Copyright © %@年 zeng. All rights reserved.\n\
//\n\n\n\
%@\n\
@interface %@ : NSObject \n\
%@\n\
...
static NSString *const kHControllerClassFileTemplate = @"\
// \n\
// %@\n\
// Created by apple on %@\n\
// \n\
// Copyright © %@年 jun. All rights reserved.\n\
//\n\n\n\
%@\n\
@interface %@ : %@ \n\
%@\n\
@end\n";
static NSString *const kMControllerClassFileTemplate = @"\
// \n\
// %@\n\
// Created by apple on %@\n\
// \n\
...
//txt文件内方法的返回内容
NSString * getTxtFileReturn(){
    
    return [NSString stringWithFormat:@"\"%@\"",getRandromString()];
}


#pragma mark:生成cpp垃圾文件
void generateSpamVCFile(NSString *vcDir,NSString *className){
    
    NSString *importString = @"#import <Foundation/Foundation.h> \n#import <UIKit/UIKit.h> \n";
    
    //一个.h头文件
    NSString *headerFilePath = [vcDir stringByAppendingPathComponent:PublicontrollHeader];
    ...
    // 写h文件 .h文件中新增注释 // mark:
    [hFileMethodsString appendFormat:@"//mark:%@ \n@property (nonatomic, copy) NSString *%@;\n\n",getRandromNoteString(),getRandomStringsWithLow(6)];
    [hFileMethodsString appendFormat:@"//mark:%@ \n@property (nonatomic, copy) NSString *%@;\n\n",getRandromNoteString(),getRandomStringsWithLow(7)];
    [hFileMethodsString appendFormat:@"//mark:%@ \n@property (nonatomic, copy) NSString *%@;\n\n",getRandromNoteString(),getRandomStringsWithLow(8)];
    
    
    //写m文件
    //system method
    [mFileMethodsString appendFormat:@"-(void)viewDidLoad{\n    [super viewDidLoad];\n        NSLog(@\"viewDidLoad\");\n    [self setupViews];\n\n}\n"];
    [mFileMethodsString appendFormat:@"-(void)didReceiveMemoryWarning {\n  [super didReceiveMemoryWarning];\n}\n\n"];
    
    NSString *viewContent = [NSString stringWithFormat:@"    UIView *baseView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 200)];\n     baseView.backgroundColor = [UIColor whiteColor];\n      [self.view addSubview:baseView];\n      UIButton *btn = [[UIButton alloc] init];\n      btn.center = baseView.center;\n     btn.backgroundColor = [UIColor redColor];\n     [btn setTitle:@\"%@\" forState:UIControlStateNormal];\n      [baseView addSubview:btn];\n    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@\"%@\" style:UIBarButtonItemStyleDone target:nil action:nil];\n",getRandromString(),getRandromString()];
    [mFileMethodsString appendFormat:@"-(void)setupViews{\n     %@\n}\n",viewContent];
    ...
    NSString *fileContent = [NSString stringWithFormat:kHControllerClassFileTemplate, fileName,getCurrentDateString(),getCurrentYearString(), importString, className, @"UIViewController",hFileMethodsString];
    [fileContent writeToFile:[vcDir stringByAppendingPathComponent:fileName] atomically:YES encoding:NSUTF8StringEncoding error:nil];
    ...
    fileContent = [NSString stringWithFormat:kMControllerClassFileTemplate, fileName, getCurrentDateString(),getCurrentYearString(), className,importClass, className,@"",className, mFileMethodsString];
    [fileContent writeToFile:[vcDir stringByAppendingPathComponent:fileName] atomically:YES encoding:NSUTF8StringEncoding error:nil];
}

#pragma mark:生成txt垃圾文件
void generateSpamTVCFile(NSString *controllerDir,NSString *className,BOOL isTable){
    
    NSString *importString = @"#import <UIKit/UIKit.h> \n";
    
    //一个.h头文件
    NSString *headerFilePath = [controllerDir stringByAppendingPathComponent:PublicontrollHeader];
    createFile(headerFilePath, importString); //创建导入所有 .h 的头文件
    ...
    // 写h文件 .h文件中新增注释 // mark:
    [hFileMethodsString appendFormat:@"//mark:%@ \n@property (nonatomic, copy) NSString *%@;\n\n",getRandromNoteString(),getRandomStringsWithLow(8).lowercaseString];
    [hFileMethodsString appendFormat:@"//mark:%@ \n@property (nonatomic, copy) NSString *%@;\n\n",getRandromNoteString(),getRandomStringsWithLow(5).lowercaseString];
    [hFileMethodsString appendFormat:@"//mark:%@ \n@property (nonatomic, copy) NSString *%@;\n\n",getRandromNoteString(),getRandomStringsWithLow(9).lowercaseString];
    ...
    [mFileMethodsString appendFormat:@"#pragma mark - life mothod\n"];
    NSString *methodName = getRandomStringsWithLow(10).lowercaseString;
    [mFileMethodsString appendFormat:@"- (void)viewWillAppear:(BOOL)animated{\n   [super viewWillAppear:animated];\n    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(%@:) name:UIKeyboardWillShowNotification object:nil];\n    }\n",methodName];
    [mFileMethodsString appendFormat:@"- (void)%@:(NSNotification *)note{\n     NSLog(@\"收到通知\");\n}\n",methodName];
    [mFileMethodsString appendFormat:@"- (void)viewWillDisappear:(BOOL)animated\n{\n    [super viewWillDisappear:animated];\n[[NSNotificationCenter defaultCenter] removeObserver:self name:UIKeyboardWillShowNotification object:nil];\n}\n"];
    
    if (arc4random() % 2) {
        [mFileMethodsString appendFormat:@"- (void)viewDidAppear:(BOOL)animated\n{      [super viewDidAppear:animated];\n}\n"];
    }
    
    if (arc4random() % 2) {
        [mFileMethodsString appendFormat:@"- (void)viewDidDisappear:(BOOL)animated\n{\n     [super viewDidDisappear:animated];\n}\n"];
    }
    
    if (arc4random() % 2) {
        [mFileMethodsString appendFormat:@"- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event\n{\n     UIView *view = touches.anyObject.view;\n    if (view == self.view) {\n  [self dismissViewControllerAnimated:YES completion:nil];\n      }\n}\n"];
    }
    ...
    if (isTable) {
        ...
        [mFileMethodsString appendFormat:@"#pragma mark - Table view data source\n"];
        [mFileMethodsString appendFormat:@"- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {\n    return %d;\n}\n\n",arc4random() % 10];
        [mFileMethodsString appendFormat:@"- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {\n     return %d;\n}\n\n",arc4random() % 10 + 1];
        
        
        NSString *randStr = getRandromString();
        NSArray *styles = @[@"UITableViewCellStyleDefault",@"UITableViewCellStyleSubtitle",@"UITableViewCellStyleValue1",@"UITableViewCellStyleValue2"];
        NSString *cellContent = [NSString stringWithFormat:@"   static NSString *%@ = @\"%@\";\n    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:%@];\n  if (cell == nil) {\n        cell = [[UITableViewCell alloc] initWithStyle:%@ reuseIdentifier:%@];\n    }\n     cell.textLabel.text = %@;\n",randStr,randStr,randStr,styles[arc4random()%4],randStr,randStr];
        
        [mFileMethodsString appendFormat:@"- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {\n   %@\n   return cell;\n}\n\n",cellContent];
        
        
        [mFileMethodsString appendFormat:@"- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{\n   [super tableView:tableView didSelectRowAtIndexPath:indexPath];\n    UITableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];\n   if(cell == nil) return;\n}"];
    }
    ...
    //写.h文件
    NSString *superClass = isTable ? @"UITableViewController" : @"UIViewController";
    NSString *fileName = [NSString stringWithFormat:@"%@.h", className];
    NSString *fileContent = [NSString stringWithFormat:kHControllerClassFileTemplate, fileName,getCurrentDateString(),getCurrentYearString(), importString, className, superClass,hFileMethodsString];
    [fileContent writeToFile:[controllerDir stringByAppendingPathComponent:fileName] atomically:YES encoding:NSUTF8StringEncoding error:nil];
